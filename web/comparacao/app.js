...
function adjustedDetail(coverage){
  ...
  if(discountPercent()<=0 || !['THIRD_PARTY','THIRD_PARTY_BASE'].includes(String(coverage?.code||'').toUpperCase())) return detail;
  const map={'100':'50','50':'35','35':'20','10':'7'};
  return detail.replace(/R\$\s*(100|50|35|10)\s*mil/gi,(_,n)=>`R$ ${map[n]} mil`);
}
...
async function init(){
  ...
  try{
    ...
    $('vehicle-summary').innerHTML=`<span>${escapeHtml(comparison.model||'Veículo')}</span>${comparison.plate?`<span>Placa ${escapeHtml(comparison.plate)}</span>`:''}<span>FIPE ${brl.format(comparison.fipeValue||0)}</span>${discountPercent()>0?`<span>Desconto ${discountPercent()}%</span>`:''}`;
