...
public class Consultant {
    ...
    private static String cleanName(String name) {
        if (name == null) throw new IllegalArgumentException("Informe o nome do consultor.");
        String clean = name.trim().replaceAll("\\s+", " ");
        if (clean.length() < 3 || clean.length() > 140) {
            throw new IllegalArgumentException("O nome do consultor deve ter entre 3 e 140 caracteres.");
        }
